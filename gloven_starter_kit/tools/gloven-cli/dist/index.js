...
async function fileFromPath2(path10, filenameOrOptions, options) {
  const stats = await fs2.stat(path10);
  return createFileFromPath(path10, stats, filenameOrOptions, options);
}
...
function loadConfig() {
  try {
    const configPath = path2.resolve(__dirname, "..", "gloven.config.yaml");
...
var OpenAIProvider = class {
  ...
  model;
  isDryRun;
  constructor(apiKey, model, isDryRun) {
    this.model = model;
  ...
  async generate(params) {
    if (this.isDryRun) {
      console.log("\u{1F3C3} DRY-RUN (no API key): Returning placeholder response");
...
var Anthropic = class extends APIClient {
   ...
   */
  constructor({ apiKey = readEnv("ANTHROPIC_API_KEY") ?? null, authToken = readEnv("ANTHROPIC_AUTH_TOKEN") ?? null, ...opts } = {}) {
    const options = {
      apiKey,
      authToken,
      ...opts,
...
function resolvePath(...segments) {
  const projectRoot = path3.resolve(__dirname3, "..", "..");
  return path3.resolve(projectRoot, ...segments);
}
...
async function loadSystemPrompt(promptDir) {
...
async function loadTaskPrompt(task, promptDir) {
  ...
  try {
    ...
    throw new Error(`Failed to load task prompt for ${task}: ${error.message}`);
  }
}
...
async function buildUserPrompt(task, promptDir, variables) {
  const taskPrompt = await loadTaskPrompt(task, promptDir);
  return injectVariables(taskPrompt, variables);
}

// src/util/costGuard.ts
init_esm_shims();
function estimateTokens(text) {
  return Math.ceil(text.length / 4);
}
function estimateCost(inputTokens, outputTokens, costPer1kInput, costPer1kOutput) {
  const inputCost = inputTokens / 1e3 * costPer1kInput;
  const outputCost = outputTokens / 1e3 * costPer1kOutput;
  return inputCost + outputCost;
}
function logCostEstimate(promptText, expectedOutputTokens, costPer1kInput, costPer1kOutput) {
...
// src/generators/plan.ts
async function generatePlan(provider, options, config, envConfig) {
  console.log("\n\u{1F4CB} Generating 30/60/90 day growth plan...");
  console.log(`\u{1F30D} Regions: ${options.regions}`);
  console.log(`\u{1F4B0} Budget: $${options.budget}`);
  console.log(`\u{1F3AF} Target: ${options.target} applications`);
  console.log(`\u{1F4C5} Timeline: ${options.days} days`);
  if (envConfig.dryRun) {
    console.log("\u{1F3C3} DRY-RUN mode: Using placeholder plan");
...
function getDryRunPlan(options) {
...
// src/generators/calendar.ts
init_esm_shims();
async function generateCalendar(provider, options, config, envConfig) {
  console.log("\n\u{1F4C5} Generating editorial calendar...");
  console.log(`\u{1F4C6} Duration: ${options.weeks} weeks`);
  if (envConfig.dryRun) {
    console.log("\u{1F3C3} DRY-RUN mode: Using placeholder calendar");
...
function getDryRunCalendar(options) {
...
| 4 | Application Tips | What reviewers look for | Twitter | Apply | 150 retweets | "\u{1F9F5} Insider secrets: What accelerator reviewers actually want to see" |
...
\u{1F9F5} THREAD: 5 mistakes every first-time founder makes (and how to avoid them)

I've reviewed 500+ startup applications. These patterns keep appearing:

1/ \u{1F4CA} Mistake #1: Building in a vacuum
...
2/ \u{1F4B0} Mistake #2: Confusing revenue with traction
...
3/ \u{1F465} Mistake #3: Hiring too fast or too slow
...
4/ \u{1F3AF} Mistake #4: Trying to serve everyone
...
**\u{1F3AF} This Week's Deep Dive: The PMF Sprint Framework**

[Link to blog post]

**\u{1F4A1} 3 Founder Interviews You Can't Miss**
...
**\u{1F4E2} Last Call: Applications Close in 2 Weeks**
...
// src/generators/partners.ts
init_esm_shims();
async function generatePartners(provider, options, config, envConfig) {
  console.log("\n\u{1F91D} Generating partner & mentor lists...");
  console.log(`\u{1F50D} Domains: ${options.domains}`);
  console.log(`\u{1F465} Target mentors: ${options.mentors}`);
  console.log(`\u{1F3E2} Target partners: ${options.partners}`);
  if (envConfig.dryRun) {
    console.log("\u{1F3C3} DRY-RUN mode: Using placeholder data");
  ...
  try {
    ...
    throw new Error(`Partner list generation failed: ${error.message}`);
  }
}
function getDryRunPartners(options) {
...
// src/generators/score.ts
init_esm_shims();

// src/zodSchemas.ts
init_esm_shims();
import { z as z2 } from "zod";
var CriterionScoreSchema = z2.object({
  score: z2.number().int().min(1).max(5).describe("Score 1-5 (1=weak, 5=exceptional)"),
  evidence: z2.string().describe("Concise evidence or quote from application"),
  strengths: z2.array(z2.string()).describe("Bullet points of strengths"),
  concerns: z2.array(z2.string()).describe("Bullet points of concerns or gaps")
});
var ApplicationScoreSchema = z2.object({
  startup_name: z2.string().describe("Company or project name"),
  ...
  weighted_total_0to100: z2.number().min(0).max(100).describe("Weighted total score (0-100 scale)"),
  decision: z2.enum(["GO", "NO_GO", "WAITLIST"]).describe("Final decision recommendation"),
  red_flags: z2.array(z2.string()).describe("Critical concerns or blockers"),
  followup_questions: z2.array(z2.string()).min(0).max(5).describe("Questions to ask in interview or follow-up"),
  summary: z2.string().min(50).max(500).describe("3-5 sentence human-readable assessment with recommendation"),
  reviewer: z2.string().optional().describe("Name of reviewer (human or AI)"),
  review_date: z2.string().optional().describe("Date of review (YYYY-MM-DD)")
});
var OutreachEmailSchema = z2.object({
  subject: z2.string().min(5).max(90).describe("Email subject line"),
  preview_text: z2.string().max(90).describe("Preview text shown in inbox (Gmail, Outlook)"),
  body_markdown: z2.string().describe(
    "Email body in Markdown format with links and bold for skimming. Use merge fields like {first_name}, {company}, {startup}"
  ),
  cta: z2.string().describe("Primary call-to-action text"),
  ps: z2.string().optional().describe("Optional postscript"),
  audience_segment: z2.enum(["founders", "mentors", "partners", "press", "investors", "alumni"]).describe("Target audience for this email"),
  tone: z2.enum(["warm", "direct", "celebratory", "urgent", "informational"]).describe("Email tone/style"),
  compliance: z2.object({
    unsub_line: z2.string().default("You received this because you signed up at gloven.org. [Unsubscribe]").describe("Unsubscribe footer text"),
    disclaimer: z2.string().optional().describe("Optional legal disclaimer")
  }).optional(),
  metadata: z2.object({
    campaign_id: z2.string().optional().describe("Campaign identifier for tracking"),
    send_date: z2.string().optional().describe("Planned send date (YYYY-MM-DD)"),
    ab_variant: z2.enum(["A", "B", "control"]).optional().describe("A/B test variant")
  }).optional()
});
function createDryRunApplicationScore(startupName) {
  ...
  return {
    ...
    summary: "This is a DRY-RUN placeholder score. The application shows potential across multiple dimensions with a weighted score of 75/100. Recommend WAITLIST pending further review and clarification of go-to-market strategy.",
    reviewer: "AI (DRY-RUN)",
...
// src/generators/score.ts
async function generateScore(provider, options, config, envConfig) {
  console.log("\n\u{1F4CA} Generating application score...");
  console.log(`\u{1F4C4} Input: ${options.input}`);
  ...
  if (envConfig.dryRun) {
    ...
    const dryRunScore = createDryRunApplicationScore(
      applicationData.startup_name || "DRY-RUN Startup"
    );
...
function generateScoreSummary(score) {
  const s2 = score;
  return `# Application Score: ${s2.startup_name}

## Overall Assessment

**Decision:** ${s2.decision}
**Weighted Score:** ${s2.weighted_total_0to100}/100
...
| Criterion | Score (1-5) |
|-----------|-------------|
| Team (25%) | ${s2.team?.score || "N/A"} |
| Problem Size (20%) | ${s2.problem_size?.score || "N/A"} |
| Insight (15%) | ${s2.insight?.score || "N/A"} |
| Traction (15%) | ${s2.traction?.score || "N/A"} |
| Timing (10%) | ${s2.timing?.score || "N/A"} |
| Fit (10%) | ${s2.fit?.score || "N/A"} |
| Impact (5%) | ${s2.impact?.score || "N/A"} |

${s2.red_flags && s2.red_flags.length > 0 ? `## \u{1F6A9} Red Flags

${s2.red_flags.map((flag) => `- ${flag}`).join("\n")}
` : ""}

${s2.followup_questions && s2.followup_questions.length > 0 ? `## Follow-up Questions

${s2.followup_questions.map((q2, i2) => `${i2 + 1}. ${q2}`).join("\n")}
` : ""}
...
// src/commands/batchScore.ts
init_esm_shims();

// src/pipelines/appIngest.ts
init_esm_shims();
...
init_esm_shims();
var EMAIL_REGEX = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g;
var PHONE_REGEX = /\b(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b/g;
var URL_REGEX = /https?:\/\/[^\s]+/g;
var API_KEY_REGEX = /\b([a-zA-Z0-9_-]{32,})\b/g;
...
function formatLogEntry(level, msg, ctx) {
...
function parseCSV(content) {
  ...
  for (let i2 = 1; i2 < lines.length; i2++) {
    const values = lines[i2].split(",").map((v2) => v2.trim());
    const app = {};
    headers.forEach((header, index) => {
      app[header] = values[index] || "";
    });
    apps.push({
      id: app.id || `app-${Date.now()}-${i2}`,
...
async function loadJSON(filePath) {
  ...
  return {
    id: data.id || path4.basename(filePath, ".json"),
...
function deduplicate(apps) {
  ...
  for (const app of apps) {
    ...
    if (companyKey && Array.from(seen.values()).some((a2) => a2.company.toLowerCase().trim() === companyKey)) {
...
async function ingestApplications(inputPath) {
  ...
  if (stats.isFile()) {
    ...
    for (const file of jsonFiles) {
      const filePath = path4.join(resolvedPath, file);
      try {
...
// src/pipelines/ranker.ts
init_esm_shims();
function calculateWeightedTotal(scored) {
  const s2 = scored.score;
  const weights = {
    team: 0.25,
    problem_size: 0.2,
    insight: 0.15,
  ...
  const normalize = (score) => (score - 1) / 4 * 100;
  const weighted = normalize(s2.team.score) * weights.team + normalize(s2.problem_size.score) * weights.problem_size + normalize(s2.insight.score) * weights.insight + normalize(s2.traction.score) * weights.traction + normalize(s2.timing.score) * weights.timing + normalize(s2.fit.score) * weights.fit + normalize(s2.impact.score) * weights.impact;
  return Math.round(weighted * 10) / 10;
}
function determineBand(score, hasRedFlags) {
  if (hasRedFlags) {
    return "NO_GO";
  }
  if (score >= 70) {
...
function rankApplications(scored) {
  logger.info(`Ranking ${scored.length} applications`);
  const ranked = scored.map((s2) => {
    const weightedTotal = calculateWeightedTotal(s2);
    const hasRedFlags = s2.score.red_flags && s2.score.red_flags.length > 0;
  ...
  ranked.sort((a2, b2) => b2.score.weighted_total_0to100 - a2.score.weighted_total_0to100);
...
async function exportLeaderboard(ranked, outputDir) {
...
*Generated by Gloven CLI*
`;
  const filePath = path5.join(outputDir, "leaderboard.md");
  await fs5.writeFile(filePath, content, "utf-8");
  logger.info(`Leaderboard exported to ${filePath}`);
}
async function exportShortlist(ranked, outputDir) {
  ...
  const filePath = path5.join(outputDir, "shortlist.csv");
  await fs5.writeFile(filePath, csv, "utf-8");
  logger.info(`Shortlist exported to ${filePath} (${goApps.length} applications)`);
}
async function exportTriage(ranked, outputDir) {
...
*Generated by Gloven CLI*
`;
  const filePath = path5.join(outputDir, "triage.md");
  await fs5.writeFile(filePath, content, "utf-8");
  logger.info(`Triage list exported to ${filePath} (${waitlistApps.length} applications)`);
}
async function exportFlags(ranked, outputDir) {
...
*Generated by Gloven CLI*
`;
  const filePath = path5.join(outputDir, "flags.md");
  await fs5.writeFile(filePath, content, "utf-8");
  logger.info(`Red flags report exported to ${filePath} (${flaggedApps.length} flagged)`);
}
async function exportAll(ranked, outputDir) {
...
// src/observability/costLedger.ts
init_esm_shims();
import fs6 from "fs/promises";
import path6 from "path";
import { existsSync as existsSync2 } from "fs";
function getLedgerPath(month, baseDir = "../../out/ledger") {
  const ledgerDir = path6.resolve(process.cwd(), baseDir);
  return path6.join(ledgerDir, `cost_${month}.jsonl`);
}
async function ensureLedgerDir(baseDir = "../../out/ledger") {
  const ledgerDir = path6.resolve(process.cwd(), baseDir);
  if (!existsSync2(ledgerDir)) {
    await fs6.mkdir(ledgerDir, { recursive: true });
  }
}
async function logCost(entry, baseDir) {
  try {
    ...
    await fs6.appendFile(ledgerPath, jsonLine, "utf-8");
  } catch (error) {
    console.warn(`\u26A0\uFE0F  Failed to log cost: ${error.message}`);
  }
}
async function getMonthlyTotal(month, baseDir) {
  try {
    const ledgerPath = getLedgerPath(month, baseDir);
    if (!existsSync2(ledgerPath)) {
      return 0;
    }
    const content = await fs6.readFile(ledgerPath, "utf-8");
    ...
    console.warn(`\u26A0\uFE0F  Failed to read ledger: ${error.message}`);
    return 0;
  }
}
...
function generateDeterministicScore(app) {
...
async function scoreApplication(provider, app, config, envConfig, runId) {
...
${JSON.stringify(applicationData, null, 2)}`;
  ...
  const costUSD = inputTokens / 1e3 * envConfig.costPer1kInput + outputTokens / 1e3 * envConfig.costPer1kOutput;
  await logCost({
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  while (retries <= maxRetries) {
    try {
      ...
      logger.warn(`Scoring failed for ${app.company} (attempt ${retries + 1}/${maxRetries + 1}): ${error.message}`);
      if (retries >= maxRetries) {
        throw new Error(`Failed to score ${app.company} after ${maxRetries + 1} attempts: ${error.message}`);
      }
      retries++;
      const backoff = Math.min(1e3 * Math.pow(2, retries) + Math.random() * 1e3, 1e4);
...
async function processWithConcurrency(items, concurrency, processor) {
  ...
  while (queue.length > 0) {
    ...
    const promises = batch.map(
      (item) => processor(item).then((result) => results.push(result)).catch((error) => errors.push({ item, error }))
    );
    await Promise.all(promises);
  }
  return { results, errors };
}
async function batchScore(provider, options, config, envConfig) {
  ...
  const scoresDir = path7.join(outDir, "scores");
  const rawDir = path7.join(outDir, "raw");
  await fs7.mkdir(scoresDir, { recursive: true });
  await fs7.mkdir(rawDir, { recursive: true });
  ...
  if (errors.length > 0) {
    logger.warn("Some applications failed to score:", {
      failed: errors.map((e2) => ({ id: e2.item.id, error: e2.error.message }))
    });
  }
  const jsonlLines = [];
  for (const { app, score } of scored) {
    const jsonPath = path7.join(scoresDir, `${app.id}.json`);
    await writeJSON(jsonPath, score);
    const mdPath = path7.join(scoresDir, `${app.id}.md`);
    const summary = generateScoreSummary2(score);
    await writeFile(mdPath, summary);
    jsonlLines.push(JSON.stringify({ id: app.id, score }));
  }
  const jsonlPath = path7.join(outDir, "scores.jsonl");
...
function generateScoreSummary2(score) {
...
${score.red_flags && score.red_flags.length > 0 ? `## \u{1F6A9} Red Flags

${score.red_flags.map((flag) => `- ${flag}`).join("\n")}
` : ""}

${score.followup_questions && score.followup_questions.length > 0 ? `## Follow-up Questions

${score.followup_questions.map((q2, i2) => `${i2 + 1}. ${q2}`).join("\n")}
` : ""}
...
function checkApiKey(envConfig) {
...
function checkFilesystem(config) {
  const promptDir = path8.resolve(config.promptDir);
  const promptDirExists = fs8.existsSync(promptDir);
  ...
  try {
    ...
    fs8.accessSync(outDir, fs8.constants.W_OK);
    outDirWritable = true;
  } catch {
...
async function performHealthCheck(config, envConfig) {
...
async function health(options, config, envConfig) {
  logger.info("Running health checks");
  const result = await performHealthCheck(config, envConfig);
  if (options.json) {
    console.log(JSON.stringify(result, null, 2));
  } else {
    console.log("\n\u{1F3E5} Gloven Health Check\n");
    ...
    let costColor = "";
    if (result.details.cost.percentUsed >= 100) costColor = "\u{1F534}";
    else if (result.details.cost.percentUsed >= 80) costColor = "\u{1F7E1}";
    else costColor = "\u{1F7E2}";
...
`;
  if (reportUrl) {
    message += `
\u{1F4C4} <${reportUrl}|View Full Report>`;
  }
  return message;
}
...
async function loadScoredApplications(outDir) {
  const scoresDir = path9.join(outDir, "scores");
  if (!existsSync3(scoresDir)) {
    logger.warn("No scores directory found");
    return [];
  }
  const files = await fs9.readdir(scoresDir);
  const jsonFiles = files.filter((f2) => f2.endsWith(".json"));
  const ranked = [];
  for (const file of jsonFiles) {
    try {
      const filePath = path9.join(scoresDir, file);
      const content = await fs9.readFile(filePath, "utf-8");
      const score = JSON.parse(content);
      const app = {
        id: path9.basename(file, ".json"),
      ...
      logger.warn(`Failed to load score from ${file}: ${error.message}`);
    }
  }
  ranked.sort((a2, b2) => b2.score.weighted_total_0to100 - a2.score.weighted_total_0to100);
  ranked.forEach((app, i2) => app.rank = i2 + 1);
  return ranked;
}
async function generateReportData(ranked, options) {
  const now = /* @__PURE__ */ new Date();
  const from = options.from || new Date(now.getTime() - 7 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  ...
  const uniqueApps = totalApps;
  const duplicates = 0;
  const averageScore = totalApps > 0 ? ranked.reduce((sum, app) => sum + app.score.weighted_total_0to100, 0) / totalApps : 0;
  const topPerformers = ranked.slice(0, 10);
  const allFlags = ranked.filter((app) => app.score.red_flags && app.score.red_flags.length > 0).flatMap((app) => app.score.red_flags || []);
  const totalFlags = allFlags.length;
  const criticalFlags = [...new Set(allFlags)].slice(0, 5);
  const currentMonth = getCurrentMonth();
  const monthToDate = await getMonthlyTotal(currentMonth);
  const budget = parseFloat(process.env.COST_BUDGET_USD || "50");
  const percentUsed = budget > 0 ? monthToDate / budget * 100 : 0;
...
function generateSparkline(distribution) {
  ...
  if (max === 0) return "\u25A1\u25A1\u25A1";
  const bars = ["\u2581", "\u2582", "\u2583", "\u2584", "\u2585", "\u2586", "\u2587", "\u2588"];
  const scale = (val) => bars[Math.min(bars.length - 1, Math.floor(val / max * bars.length))];
...
`;
  if (Object.keys(data.intake.byRegion).length > 0) {
    md += `**By Region:**
`;
    for (const [region, count] of Object.entries(data.intake.byRegion).sort((a2, b2) => b2[1] - a2[1])) {
      md += `- ${region}: ${count}
`;
    }
    md += `
`;
  ...
  if (Object.keys(data.intake.byStage).length > 0) {
    md += `**By Stage:**
`;
    for (const [stage, count] of Object.entries(data.intake.byStage).sort((a2, b2) => b2[1] - a2[1])) {
      md += `- ${stage}: ${count}
`;
    }
    md += `
...
async function report(options, config, envConfig) {
  logger.info("Generating health report");
  const outDir = resolvePath(config.outDir);
  const reportsDir = path9.join(outDir, "reports");
  await fs9.mkdir(reportsDir, { recursive: true });
  ...
  const reportPath = path9.join(reportsDir, "gloven_health_report.md");
...
program.name("gloven").description("AI-powered growth CLI for Gloven accelerator").version("1.0.0");
program.command("plan").description("Generate 30/60/90 day founder acquisition plan").requiredOption("--regions <regions>", 'Target regions (e.g., "EMEA,LATAM")').requiredOption("--budget <amount>", "Monthly budget in USD").requiredOption("--target <number>", "Target number of applications").requiredOption("--days <number>", "Timeline in days (typically 90)").requiredOption("--out <path>", "Output file path (e.g., ../../out/plan.md)").action(async (options) => {
  try {
...
program.command("calendar").description("Generate editorial calendar with content samples").requiredOption("--weeks <number>", "Number of weeks to plan").requiredOption("--out <path>", "Output file path (e.g., ../../out/editorial_calendar.md)").action(async (options) => {
  try {
    const { config, env } = getConfig();
    const provider = createProvider(env);
...
program.command("partners").description("Generate partner and mentor target lists with outreach templates").requiredOption("--domains <domains>", 'Focus domains (e.g., "SaaS,AI/ML,FinTech")').requiredOption("--mentors <number>", "Number of mentor targets").requiredOption("--partners <number>", "Number of partner targets").requiredOption("--out <path>", "Output file path (e.g., ../../out/partners.md)").action(async (options) => {
  try {
    const { config, env } = getConfig();
    const provider = createProvider(env);
...
program.command("score").description("Score startup application using Gloven rubric").requiredOption("--input <path>", "Input JSON file with application data").requiredOption("--out <path>", "Output file path (e.g., ../../out/score.json)").action(async (options) => {
  try {
    const { config, env } = getConfig();
    const provider = createProvider(env);
...
program.command("batch-score").description("Score multiple applications efficiently").requiredOption("--input <path>", "Input file or directory (e.g., ../../data/apps/applications.csv)").option("--concurrency <number>", "Concurrent scoring operations", "3").option("--out <path>", "Output directory", "../../out").option("--dry-run", "Use DRY_RUN mode with deterministic placeholder scores").action(async (options) => {
  try {
    const { config, env } = getConfig();
    if (options.dryRun) {
      env.dryRun = true;
...
program.command("health").description("Check system health and readiness").option("--json", "Output as JSON").action(async (options) => {
  try {
    const { config, env } = getConfig();
...
program.command("report").description("Generate weekly health report").option("--notify", "Send Slack notification").option("--from <date>", "Start date (YYYY-MM-DD)").option("--to <date>", "End date (YYYY-MM-DD)").action(async (options) => {
  try {
    const { config, env } = getConfig();
